...
class ELmo():
    def __init__(self,file,search):
        ...
        embed = hub.Module(url) #importing the pretrained model

        embeddings = embed(
            sentences,
            signature="default",
            as_dict=True)["default"]            #extracting the word vectors of thesentences we get from pdf ssummary
        ...
        for i, j in cosine_similarities.nlargest(3).iteritems():
            ...
            for i in sentences[i].split():
                ...
                """if i.lower() in search_string:
                    ...
                    output += " " +str(i)"""
            output += "\n\n"
        with open(search_string, "a+") as f:
                f.write(output + "\n")
        subprocess.Popen(["notepad.exe", search_string])
        print(output)